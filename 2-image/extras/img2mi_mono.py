...
# Translate monochromatic image to Gowin Memory Initialization file
...
def convert_mono(im):
    """ Read image. FIX: Assume 8bit address for cols and lines. """

    print(miheader(depth(im), 1))
    
    px = im.load()
    for lin in range(im.size[1]):
        for col in range(im.size[0]):
            if px[col, lin] > 0:
                print("%02x%02x:%d" % (lin, col, 1))
