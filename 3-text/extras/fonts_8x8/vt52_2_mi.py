# Usage:
#  python $0 > vt52.mi
# VT52 font from https://github.com/fritzm/vt52/tree/master
...
print("#Address_depth=", output_chars * output_rows * output_cols)
print("#Data_width=1")


# There is not a more inefficient way to do it
for char in range(0, output_chars):
    for row in range(0, output_rows):
        for col in range(0, output_cols):
            if row > font_rows - 1:
                out = False
            elif col > font_cols - 1:
                out = False
            elif char > font_chars -1:
                out = False

            else:
                row_byte = font_data[char*font_rows + row]
                col_bit = row_byte & (1 << 7-col)
                out = col_bit
